     ...
     3          EX(ml),gamionorbX(ml),gamheatiorb(ml),torvhat(ml),
     ...
     2          xlpmom(ml),gammahat(ml),qhatheati(ml),vph1(ml),
     3          vpol1ex(ml),xnutheta1(ml),xnutheta2(ml),xnudrageff1(ml),
     4          vtordhat(ml),xnudrageff2(ml)

      DIMENSION EMIN(ML,ML),EPMIN(ML,ML)

      double precision TEDP,XLZ2DP,DLDT2DP,Tdbl,xlzdbl,dlzdbl
      real Snbi,dFdr,Edr,BCnbi,BCenbi,gammax,inter,vph2,vph1,forbl
...
c     1     (velthet1(n)-velthet2(n))/(fp*eq(1)*bthet)
c     vphizterm(n) =xmas(1)*xnuc12(n)*torv(n)/(eq(1)*bthet)
c     erterm(n) =xmas(1)*(xnuc12(n)+ynudrag1(n))*
c     1     ((erada(n)/bthet)+velthet1(n)/fp)/(eq(1)*bthet)
c     ***************************************************
...
c     if(ioptpinchi.eq.5) then
c     vpinchi(n) = (-1.*xmomtor1(n)/yni(n,1) - eq(1)*atnum(1)*ephia +
...
c     brack(j) = 0.0
     ...
     1      ynud2*brack(j))/(ynud1+ynud2)
          vtor1(j) = vtor1(j) + (atnum(1)*eq(1)*(bthet*gamion(j,1) +
     2    xni(1)*ephia*atnum(1))+zbar2(j)*eq(1)*xni(2)*ephia)/
...
c     1  (y2+xni(2)*zbar2(n)*eq(1)*ephia)/(xni(2)*xmas(2)*xnuc(2,1)*ynud2)
c     vtor2(n) = (1.+ynud1)*vtor1(n) -
c    1      (atnum(1)*eq(1)*(bthet*gamion(n,1)+xni(1)*ephia) + y1)/
c   2 (xni(1)*xmas(1)*xnuc(1,2))
          vtor2(j) = vtor1(j)/(1.+ynud2) +
     1    (atnum(2)*eq(1)*xni(2)*ephia + y2)/(xni(1)*xmas(1)*xnuc(1,2))
...
c     ***************************************************************
      ...
      GAMION(25,1) = enh*FLUXPART
c     gamion(25,2) = 0.03*gamion(25,1)
      gamion(25,2) = 0.0
      xnsol = yni(25,1)
...
c     GAMION(N,1) = GAMION(N+1,1)*(1.+delma/radius_int)
c     1              -dens(n)*xnuionb(n)*delma
c     2                 -DENS(N)*XNUIONI(N)*DELMA
c     3            *(1.+fracz*zbar2(n))
cc     gamion(n,2) = 0.03*gamion(n,1)*(1-cforbl(n))
c     GAMION(N,2) = GAMION(N+1,2)*(1.+delma/radius_int)
c     1           - DENS(N)*XNUIONb(N)*DELMA*(1.+fracz*zbar2(n))
c     ssion(n) =  yni(N,1)*XNUIONI(N)*(1.+fracz*zbar2(n))
cc     1                + yni(n,1)*xnuionb(n)
c     call edgerotran(n,10)
c**********Integral cylindrical particle flux******************************
        call edgerotran(n,10)
c       srprim = dens(n+1)*xnuionb(n+1)+dens(n+1)*xnuioni(n+1)*
c     1       (1.+fracz*zbar2(n+1))
        srprim = dens(n+1)*xnuioni(n+1)*(1.+fracz*zbar2(n+1))+Snbi(n+1)
        GAMION(N,1) = (rhor(n+1)/rhor(n))*(GAMION(N+1,1)-srprim*delma)
        GAMION(N,2) = (rhor(n+1)/rhor(n))*(GAMION(N+1,2) - DENS(N+1)*
     1               XNUIONb(N+1)*DELMA*(1.+fracz*zbar2(n+1)))
c***********Integral cylindrical heat flux******************
c     srprimq = -cxcool(n+1)-cmulteq*qie(n+1)+fionb(n)*qnb(n+1)
c      gamheati(n) = (rhor(n+1)/rhor(n))*(gamheati(n+1)-srprimq*delma)
...
c     Corrent BCs for fast ions and energy lost in the core (see soldiv.for)
      gammahat(1) = BCnbi + dens(1)*xnuioni(1)*(1.+fracz*zbar2(1))*
     1     0.02*radminor
      write(*,*) 'radminor =', radminor
      write(*,*) 'gamion(1,1) =', gamion(1,1)
      write(*,*) 'gamion(25,1) =', gamion(25,1)
      write(*,*) 'gammahat(1) =', gammahat(1)
...
c     gammahat(n) = gammahat(1)*exp(-2.*forbl(n))*rhor(1)/rhor(n)
c     qhatheati(n) = qhatheati(1)
c     do 555 m=2,n
c        call edgerotran(n,10)
c         xpon = exp(-2.*(forbl(n)-forbl(m)))
c        srprim = (dens(m)*xnuionb(m)+dens(m)*xnuioni(m)*
c     1       (1.+fracz*zbar2(m)))
c      if (n.eq.m) then
c         inter = srprim*delma
c      else
c           inter = xpon*srprim*delma*rhor(m)/rhor(n)
c      endif
c        gammahat(n) = gammahat(n)+inter
c        call edgerotran(m,10)
c        xponq = exp(-2.*(eorbl(n)-eorbl(m)))
c        srprimq = -cxcool(m) - cmulteq*qie(m)
c     1             +fionb(m)*qnb(m)
c       if (n.eq.m) then
c         interq = srprimq*delma
c      else
c          interq = xponq*srprimq*delma*rhor(m)/rhor(n)
c      endif
c        qhatheati(n) = qhatheati(n)+interq
c555  continue
cc     Assume radial carbon flux is zero
c      gamion(n,2) = 0.0
cc      gamion(n,2) = 0.03*gammahat(n)*(1-cforbl(n))
c********
c**********New Integral form in cylindrical geometry******************************
        xpon = exp(-2.*(forbl(n)-forbl(n-1)))
        srprim = Snbi(n)+dens(n)*xnuioni(n)*
     1         (1.+fracz*zbar2(n))
        xnuionb(n) = Snbi(n)/dens(n)
        gammahat(n)=(rhor(n-1)/rhor(n))*gammahat(n-1)*xpon+srprim*delma
c     write(*,*) 'n =', n
c     write(*,*) 'Snbi =', Snbi(n)
c     write(*,*) 'dens =', dens(n)
c     write(*,*) 'xnuioni =', xnuioni(n)
c**********New Integral cylindrical heat flux******************************************
        qhatheati(n) = qhatheati(1)
        xponq = exp(-2.*(eorbl(n)-eorbl(n-1)))
        srprimq = -cxcool(n)-cmulteq*qie(n)
     1            +fionb(n)*qnb(n)
        qhatheati(n)=(rhor(n-1)/rhor(n))*qhatheati(n-1)*xponq+srprimq*delma
...
c     vrad1(n) = gamion(n,1)*ziol/yni(n,1)
        vrad1(n) = gammahat(n)/yni(n,1)
        vrad2(n) = gamion(n,2)/yni(n,2)
...
c     y11 = xmtor(1)+atnum(1)*xk*(xni(1)*ephia+bthet*gamion(n,1))
c     y11 below is with gammahat calculation
        y11 = xmtor(1)+atnum(1)*xk*(xni(1)*ephia+bthet*gammahat(n))
...
c***********************************************************
          zp100 = 1. - 2.*fop(n)
          y11 = xmtor(1)+atnum(1)*xk*(xni(1)*ephia+bthet*gamion(n,1)*zp100)
          y1 = y11/(xni(1)*xmas(1)*xnuc(1,2))
        endif
c********************************************************************************
        y22 = xmtor(2) + zbar2(n)*xk*(xni(2)*ephia + bthet*gamion(n,2))
...
c     update to include effects of IOL -- July 2014
        if(ndrag.eq.3) then

          ZR = 1.0
          DELV1(N) = 0.
          XNUDTOT1(N) = (Y11+Y22)/((xni(1)*xmas(1)+
     1    ZR*xni(2)*xmas(2))*TORV(N))
...
c     approximation #5, extention of #3 (Vj-Vk)_0 with IOL for Carbon only
c     N.Piper 9/2015
        if(ndrag.eq.5) then
          DELV0(N) = 0.
          xnudrageff1(n) = 0.
          TORVHAT(N) = TORV(N)
          xnudrageff1(n) = (Y11+Y22-xni(1)*xmas(1)*xnudrageff1(n)*DELV0(N))/
     1     ((xni(1)*xmas(1)+xni(2)*xmas(2))*TORVHAT(N))
          DELV0(N) = (Y11-xni(1)*xmas(1)*xnudrageff1(n)*TORVHAT(N))/
     1     (xni(1)*xmas(1)*(xnuc(1,2)+xnudrageff1(n)))
          xnudrageff2(N) = (Y22+xni(2)*xmas(2)*xnuc(2,1)*DELV0(N))/
     1     (xni(2)*xmas(2)*TORVHAT(N))
          DELV1(N) = DELV0(N)
          XNUDTOT1(N) = xnudrageff1(n)
          XNUDTOT2(N) = xnudrageff2(n)

c     separate drag frequencies for the two species from individ mom balances
          XNUDTOT1s(N) = (Y11-XNI(1)*XMAS(1)*XNUC(1,2)*DELV1(N))/
     2    (XNI(1)*XMAS(1)*(TORVHAT(N)+DELV1(N)))
          XNUDTOT2s(N) = (Y22+XNI(2)*XMAS(2)*XNUC(2,1)*DELV1(N))/
     2    (XNI(2)*XMAS(2)*TORVHAT(N))
          vtor1(n) = torv(n) + delv1(n) + yy1(n) - yy2(n)
          vtor2(n) = torv(n)
          vtordhat(n) = vtor1(n) - yy1(n)
        endif

c   #6, Directly calculate drag frequencies from individ mom balances (must have deuterium data)
c   Apply intrinsic rotation correction and calculate DELV
c    N.Piper 10/2015
        if (ndrag.eq.6) then
          TORVHAT(N) = TORV(N)-YY2(N)
          VTORDHAT(N) = VTORD(N)-YY1(N)
          DELV1(N) = VTORDHAT(N)-TORVHAT(N)
          XNUDTOT1s(N) = (Y11-XNI(1)*XMAS(1)*XNUC(1,2)*DELV1(N))/
     2    (XNI(1)*XMAS(1)*(TORVHAT(N)+DELV1(N)))
          XNUDTOT2s(N) = (Y22+XNI(2)*XMAS(2)*XNUC(2,1)*DELV1(N))/
     2    (XNI(2)*XMAS(2)*TORVHAT(N))
          XNUDTOT1(N) = XNUDTOT1s(N)
          XNUDTOT2(N) = XNUDTOT2s(N)
          vtor1(n) = VTORD(N)
          vtor2(n) = torv(n)
        endif

c   Approximation #7, Perturbation Theory using only experimental values
        if (ndrag.eq.7) then
          xnudrageff1(n) = (Y11+Y22)/((xni(1)*xmas(1)+
     1    xni(2)*xmas(2))*torv(n))
          DELV0(N) = (Y11-xni(1)*xmas(1)*xnudrageff1(n)*TORV(N))/
     1    (xni(1)*xmas(1)*(xnuc(1,2)+xnudrageff1(n)))
          xnudrageff2(N) = (Y22+xni(2)*xmas(2)*xnuc(2,1)*DELV0(N))/
     1    (xni(2)*xmas(2)*TORV(N))
c     Calculate nudrag_d^1 from combined momentum balance where nudrag_d,j=nudrag^0
          XNUDTOT1(N) = (Y11+Y22-xni(1)*xmas(1)*xnudrageff1(n)*DELV0(N))/
     1      ((xni(1)*xmas(1)+xni(2)*xmas(2))*TORV(N))
          DELV1(N) = DELV0(N)
c     Use individual momentum balances with DELV1 to find drag frequencies
          XNUDTOT1s(N) = (Y11-XNI(1)*XMAS(1)*XNUC(1,2)*DELV1(N))/
     2    (XNI(1)*XMAS(1)*(TORV(N)+DELV1(N)))
          XNUDTOT2s(N) = (Y22+XNI(2)*XMAS(2)*XNUC(2,1)*DELV1(N))/
     2    (XNI(2)*XMAS(2)*TORV(N))
c     Set nudrag_k to the one calculated from momentum balance
          XNUDTOT2(N) = XNUDTOT2s(N)
          XNUDTOT1(N) = XNUDTOT1s(N)
          VTORD(N) = TORV(N) + DELV1(N)
          vtor1(n) = VTORD(N)
          vtor2(n) = torv(n)

        endif

c     Approximation #8, Perturbation theory using V_j^exp - V_k^exp = V_j^IOL - V_k^IOL
C   N.Piper 10/2016
        if (ndrag.eq.8) then
          DELV0(N) = yy1(n)-yy2(n)
          xnudrageff1(n) = (Y11+Y22)/((xni(1)*xmas(1)+
     1    xni(2)*xmas(2))*TORV(N)+xni(1)*xmas(1)*DELV0(N))
c            write(*,*) 'n =', n
c            write(*,*) 'Y11+Y22 =', (Y11+Y22)
c              write(*,*) 'term 1', (xni(1)*xmas(1)+xni(2)*xmas(2))*TORV(N)
c              write(*,*) 'term 2', xni(1)*xmas(1)*DELV0(N)
c              write(*,*) 'nudragfactor =', xnudrageff1(n)/(Y11+Y22)
c              write(*,*) 'xnudrageff =', xnudrageff1(n)
c              write(*,*) 'den1 =', xni(1)
c              write(*,*) 'DELV0(N) =', DELV0(N)
c              write(*,*) 'TORV(N) =', TORV(N)
          DELV0(N) = (Y11 - xni(1)*xmas(1)*xnudrageff1(n)*torv(n))/
     1    (xni(1)*xmas(1)*(xnuc(1,2)+xnudrageff1(n)))
          xnudrageff2(N) = (Y22+xni(2)*xmas(2)*xnuc(2,1)*DELV0(N))/
     1    (xni(2)*xmas(2)*TORV(N))
          XNUDTOT1(N) = (Y11+Y22-xni(1)*xmas(1)*xnudrageff1(n)*
     1    DELV0(N))/((xni(1)*xmas(1)+xni(2)*xmas(2))*TORV(N))
          DELV1(N) = (Y11 - xni(1)*xmas(1)*xnudtot1(n)*torv(n))/
     1    (xni(1)*xmas(1)*(xnuc(1,2)+xnudtot1(n)))
c            write(*,*) 'XNUDTOT1(N) =', XNUDTOT1(N)

c   Use individual momentum balance to find drags frequencies
          XNUDTOT1s(N) = (Y11-XNI(1)*XMAS(1)*XNUC(1,2)*DELV1(N))/
     2    (XNI(1)*XMAS(1)*(TORV(N)+DELV1(N)))
          XNUDTOT2s(N) = (Y22+XNI(2)*XMAS(2)*XNUC(2,1)*DELV1(N))/
     2    (XNI(2)*XMAS(2)*TORV(N))
          ...
          VTORD(N) = TORV(N) + DELV0(N)
          vtor1(n) = VTORD(N)
          vtor2(n) = torv(n)
        endif

c   #9, Directly calculate drag frequencies from individ mom balances (must have deuterium data) w/o subtracting IOL terms
C   N.Piper 10/2016
        if (ndrag.eq.9) then
          DELV1(N) = VTORD(N) - TORV(N)
          XNUDTOT1s(N) = (Y11-XNI(1)*XMAS(1)*XNUC(1,2)*DELV1(N))/
     1    (XNI(1)*XMAS(1)*VTORD(N))
          XNUDTOT2s(N) = (Y22+XNI(2)*XMAS(2)*XNUC(2,1)*DELV1(N))/
     1    (XNI(2)*XMAS(2)*TORV(N))
          XNUDTOT1(N) = XNUDTOT1s(N)
          XNUDTOT2(N) = XNUDTOT2s(N)
          xnudrageff1(n) = XNUDTOT1s(N)
          xnudrageff2(n) = XNUDTOT2s(N)
          DELV0(N) = DELV1(N)
          vtor1(n) = VTORD(N)
          vtor2(n) = TORV(N)
        endif
...
c     6/26/07 version neglects poloidal asymmetries
        ...
        rho_main = vth(1)/(eq(1)*bphi/xmas(1))
        rho_imp = vth(2)/(atnum(2)*eq(1)*bphi/xmas(2))
...
c     *****extend the vrad source terms to included the heat flux viscosity contribution***********
        cvisc = 1.
        s1 = eq(1)*vrad1(n)*bphi/xmas(1)
c           s1 = s1 + cvisc*(vth(1)*qedge(n)*f1/(rmajor))*
c     2     (xki*xk*xti(n)/(exlti(n)*eq(1)*bphi))
        s2 = atnum(2)*eq(1)*vrad2(n)*bphi/xmas(2)
c     s2 = s2     + cvisc*(vth(2)*qedge(n)*f2/(rmajor))*
c     2     (xkz*xk*xti(n)/(exlti(n)*eq(1)*bphi))
c     ************************************************************************************************
...
c*************calculate Vtheta from eq (6) of structure paper --spring 2013********
        ...
        xnutheta1(n)= xnuvisc1 + xnuions1 + xnucxs1 +
     2  xnuc(1,2)*qedge(n)*rmajor/vth(1)
        xnutheta2(n)= xnuvisc2 + xnuions2 + xnucx2 +
     2       xnuc(2,1)*qedge(n)*rmajor/vth(2)
        zv = 1.0
        drive1v(n) = xnuvisc1*xki*ti(n)/(3.*(bphi**2)*exlti(n))
c     drive1(n) = zv*drive1v(n) +
c     2(eq(1)*gamion(n,1)**qedge(n)*rmajor)/(yni(n,1)*xmas(1)*vth(1))
c     drive1 below is with gammahat
        drive1(n) = zv*drive1v(n) +
     2  (eq(1)*gammahat(n)*qedge(n)*rmajor)/(yni(n,1)*xmas(1)*vth(1))
        drive2v(n) = xnuvisc2*xkz*ti(n)/(3.*zbar2(n)*(bphi**2)*exlti(n))
        drive2(n) = zv*drive2v(n)+zbar2(n)*eq(1)*gamion(n,2)
     3  *qedge(n)*rmajor/(yni(n,2)*xmas(2)*vth(2))
        xnuc12star = (qedge(n)*rmajor*xnuc(1,2))/vth(1)
        xnuc21star = (qedge(n)*rmajor*xnuc(2,1))/vth(2)
        bracc = 1.-xnuc12star*xnuc21star/(xnutheta1(n)*xnutheta2(n))
c     old vpol calculation for Eq (6) in structure paper (2013)
c     vpol61(n) = -1.*bphi*(drive1(n) +
c     2           xnuc12star*drive2(n)/xnutheta2)/bracc
c     vpol62(n) = -1.*bphi*(drive2(n) +
c     2     xnuc21star*drive1(n)/xnutheta1)/bracc
c     new calculation
        vpol61(n) = (-1.*bphi/xnutheta1(n))*(drive1(n) +
     2  xnuc12star*drive2(n)/xnutheta2(n))/bracc
        vpol62(n) = (-1.*bphi/xnutheta2(n))*(drive2(n) +
...
c     CHIIorbX15(N) = EXLTI(N)*(((GAMHEATI(N)*orbex)/(XNION*XK*XTI(N)))
c     1                       -     1.5*GAMION(N,1)*orbfx/XNION)

c     ion particle fluxes reduced by ion orbit and X loss
        gamionorb(n) = gamion(n,1)*(1.-forbl(n))
        cdionorb(n) = eq(1)*gamion(n,1)*forbl(n)
...
c      ipolopt = 1 uses vthet2 = vexp, =0 calculates vthet2
        ...
        erada(n) = erex(n)
        zbeam =  -1.0*xmomtor1(n)/(yni(n,1)*eq(1)*bthet)
...
c     if(vp2.gt.0.0) vp2 = 0.0
     ...
     1  (velthet1(n)-velthet2(n))/fp)/(eq(1)*bthet)
...
c**********************************************************************
     ...
     1  ((velthet1(n)/fp)))/ (eq(1)*bthet)

        zfrict = 0.
        zdrag = eradterm+vtheterm
        vphiterm = -1.*xmas(1)*xnuc12(n)*(torv(n)-yy2(n))/(eq(1)*bthet)
...
c*******************************************************************
        zdrag = eradterm+vtheterm
        vphiterm = -1.*xmas(1)*xnuc12(n)*torv(n)/(eq(1)*bthet)
...
c     exd11(n) = xmas(1)*(xnuc12(n)+ynudrag1(n))*xti(n)/
c     1                 (eq(1)*(bthet**2))
c     updated diffusion coefficient 7/15/15 from Eq.5 NF 54,114020 (2012)
        exd11(n) = xmas(1)*xnuc12(n)*xti(n)*(1+(ynudrag1(n)/xnuc12(n))
     1  -(1./6.))/(eq(1)*(bthet**2))
...
c     shear suppression of chierb
        ...
        xlrb=6.28*qedge(n)*rmajor*sqrt(2.*exne(n)*(eq(1)**2)*res*rhot/
...
c     1  (clight*rhote/(eq(1)*bfield))*xk*xte(n)*
...
c  goto 6799
c  xnum = xni(1)*eq(1)*atnum(1)*bthet*vrad1(n) + xmtor(1) +
c    1             xmtor(2) + (xni(1)*atnum(1)+xni(2)*zbar2(n))*eq(1)*ephia
        bracket = (velthet1(n)-velthet2(n))/fp + ti(n)*xlpm(n)*
     1            (1.-atnum(1)/zbar2(n))/(atnum(1)*bthet)
        xnum = (xmtor(1)/xni(1)+xk*ephia+xk*bthet*vrad1(n))/
     1   xmas(1)-xnuc12(n)*bracket
c     xdenom = (xni(1)*xmas(1)+xni(2)*xmas(2))*torv(n) +
c    1  xni(1)*xmas(1)*bracket
        xdenom = torv(n) + bracket
        xnud1(n) = xnum/xdenom
        xnudtot1(n) = xnud1(n)
        vphical(1) =((xmtor(1)/xni(1)+xk*ephia+xk*bthet*vrad1(n))/xmas(1)
     1              +xnuc21(n))/(xnuc12(n)+xnud1(n))
        xnum2 = (xmtor(2)/xni(2)+zbar2(n)*(xk*ephia+xk*bthet*vrad2(n)))
...
c     2  (ynud(1)+ynud(2))
c     vphical(1) = vphical(1) + (atnum(1)*eq(1)*(bthet*gamion(n,1) +
c     2        xni(1)*ephia)+zbar2(n)*eq(1)*xni(2)*ephia)/
c     3       (xni(1)*xmas(1)*xnuc(1,2)*(ynud(1) + ynud(2)))
c    vphical(2) = (1.+ynud(1))*vphical(1) -
c     1     (atnum(1)*eq(1)*(bthet*gamion(n,1)+xni(1)*ephia) + xmtor(1))/
c     2     (xni(1)*xmas(1)*xnuc(1,2))
        vtor1(n) = vphical(1)
        vtor2(n) = vphical(2)
        vphia(1) = vphical(1)
 ...
 750   continue
     ...
     1 gamionorbCUR gamionC   xgamionorb  xtransink  xtransourc'
      do 751 n = 1,25
        ...
        write(121,1000) rhor(n),gamion(n,1), gammahat(n),gamionorb(n),
     1  gamionorbCUR(n),gamion(n,2), xgamionorb(n),xtransink(n),
     2  xtransource(n)
 751  continue
      Write(121,'(1x,35A)')'   rho     gamheati   heatiorb qhatheati
     1 heatiXtran heatiXorb '
      do 752 n = 1, 25
        write(121,1000)rhor(n),gamheati(n),gamheatiorb(n),qhatheati(n),
 ...
 7546  continue
     ...
     1heatinerti heatvisci gamconvi  '
      do 755 n = 1,25
        ...
        eta4(n) = yni(n,1)*xmas(1)*xti(n)*xk/(eq(1)*bfield)
...
c     update to include effects of IOL (7/2014): torv(n)--> torv(n)-yy2(n)-delv1(n)
        velthet3(n) = fp*(vtor1(n)-yy1(n)-delv1(n)) -erex(n)/bphi -
     2                ti(n)*xlpm(n)/bphi
     ...
     2  xnzexp*xmas(2)*xnudtot2(n)/zbar2(n))/rhomassd
c     Extran = 0.0
c     Eionorb = 0.0
...
c     2 vphi1ex    DELV0   DELV1    vphi1sep   vphi2sep '
      write(121,'(1x,35A)')'  rho    nudragj_0   nudragk_0
     1  DELV0     DELV1   nuinert1   nucx  '

c   Temporary values set for vph, vphiex, and vphip variables.
      do 762 n = 1,25
...
c     2           (yni(n,1)*xmas(1)*xnudtot1s(n))
c      vphi2p=(torq2(n)+yni(n,2)*xmas(2)*xnuc21(n)*delv1(n))/
c     2           (yni(n,2)*xmas(2)*xnudtot2s(n))
c   this is vphi1p & vphi2p of p86 notes calculated w/sep mom bal and sep nud1 & nud2
c     torq1(n) = torq1(n)/(yni(n,1)*xmas(1)*(xnuc12(n)+xnudtot1(n)))
c     torq2(n) = torq2(n)/(yni(n,2)*xmas(2)*(xnuc21(n)+xnudtot2(n)))
c     xx = (xnuc12(n)+xnudtot1(n))*(xnuc21(n)+xnudtot2(n))
c     brackx = 1. -  xnuc12(n)*xnuc21(n)/xx
c     vph1(n)=(torq1(n)+xnuc12(n)*torq2(n)/(xnuc12(n)+xnudtot1(n)))
c     1      /brackx
c     vph1(n) = vph1(n)+(bphi/sqrt(bphi**2+bthet**2))*yy1(n)
c     vph2=(torq2(n)+xnuc21(n)*torq1(n)/(xnuc21(n)+xnudtot2(n)))/brackx
c     vphi1ex = torv(n) + delv1(n)-yy1(n)
c     vphi1ex = torv(n) + delv1(n)
c  this is vphi1 & vphi2 on p70 of notes calc w/ nud1=nud2=nud0
c     write(121, 1000) rhor(n),vph1,vph2,torv(n),vphi1ex,DELV0(N),
c     2     DELV1(N),vphi1p, vphi2p
c7615 continue
      write(121,'(1x,35A)')'  rho        rotj      rotk   eradpres
     2eradtot  eradex  '
...
c*********Eq 4 from Structure... paper June, 2013*******************
      do 7615 n = 1,25
        ...
        vph1(n)=(torq1(n)+xnuc12(n)*torq2(n)/(xnuc12(n)+xnudzero(n)))
     1          /brackx
        vph1(n) = vph1(n)+abs(bphi/sqrt(bphi**2+bthet**2))*yy1(n)
        vph2=(torq2(n)+xnuc21(n)*torq1(n)/(xnuc21(n)+xnudzero(n)))/brackx
        vph2 = vph2+abs(bphi/sqrt(bphi**2+bthet**2))*yy2(n)
        vphi1ex = torv(n) + delv1(n)+yy1(n)
c     poloidal velocity from structure paper eq 26
        vpol1ex(n) = 1./bphi*(bthet*vphi1ex-erex(n)-1./(eq(1)*yni(n,1))
     1            *xk*ti(n)*yni(n,1)*xlpm(n))
c***************calculate Erad from Eq 7 of structure paper--June, 2013****
        xjk = yni(n,1)*xmas(1)/(yni(n,2)*xmas(2))
c     theoretical deuterium
c     rotj = (vpol61(n)*bphi-vph1(n)*bthet)/(1. + (1./xjk))
c     experimental deuterium
        rotj = (vpol1ex(n)*bphi-vphi1ex*bthet)/(1. + (1./xjk))
        if(ioptDdata.eq.1)then
          rotj = (vpold(n)*bphi-vtord(n)*bthet)/(1. + (1./xjk))
        endif
c     theoretical carbon
c     rotk = (vpol62(n)*bphi-vph2*bthet)/(1. + xjk)
c     experimental carbon
        rotk = (vthexp(n)*bphi-torv(n)*bthet)/(1. + xjk)
        if(ioptDdata.eq.1)then
          rotk = (vthexp(n)*bphi-vtord(n)*bthet)/(1. + xjk)
        endif
        eradrot = -(rotj+rotk)
c****resistivity****************
        xloglambda = 18.0
     ...
     2            (yni(n,1)+zbar2(n)*yni(n,2))
        ...
        etares  = vnum/vdenom
c*******************************
        orbcur = gamion(n,1)-gammahat(n)
c     eradcur = -1.*eq(1)*etares*(orbcur+Fretcur(n))
        eradcur = -1.*eq(1)*etares*orbcur
        pressjk = xk*(yni(n,1)+yni(n,2))*ti(n)
        eradpres= -1.*pressjk*xlpm(n)/(eq(1)*(yni(n,1)+zbar2(n)*yni(n,2)))
...
c     velthet2(25) = vtheta(2)
 ...
 808       continue
          ...
          vpinchi(25) = -1.*xmomtor1(25)/yni(25,1)  +  eq(1)*ephia
     ...
     1     (xnudtot1(25)+xnuc12(25))*(eradfb(25)/bthet+velthet1(25)/fp)
          ...
          vpinchi(25) = vpinchi(25)/(eq(1)*atnum(1)*bthet)
