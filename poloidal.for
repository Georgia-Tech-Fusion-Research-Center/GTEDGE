...
c     goto 1003
      if(ioptedge.eq.1)       then
        bfield = bphi
        x1 = atnum(1)*eq(1)*bfield/(xmas(1)*VTH(1))
...
c     vrhat(1) = 0.0
        ...
        xmomhat(1) = z1*y1
        x2 = atnum(2)*eq(1)*bfield/(xmas(2)*VTH(2))
...
c     cosine moment species 1
     ...
     1  vc2(1)-0.5*(EP**2)*cf*FP*XNUSTAR(1,2)*VTHETa(2)*SQRT(XMAS(1)
     2        /XMAS(2))
