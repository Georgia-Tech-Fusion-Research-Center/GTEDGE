...
C         CYCLOTRON FREQUENCIES & GYRORADII
      DO 10 J = 1,2
        OMII(J) = ATNUM(J)*xk*abs(BPHI)/XMAS(J)
c     if(j.eq.2) omii(j) = zbar2(nmesh)*xk*abs(bphi)/xmas(j)
...
C             FREQUENCIES
          ...
          XNUC(J,K)=XNI(K)*(ATNUM(K)**2)*(ATNUM(J)**2)*(EQ(1)**2.5)
     1         *COULOG(J,K)/(47.25*SQRT(XMR)*(EP0**2)*(TEMP(J)**1.5))
...
c 45   DELP = (TEMP(1)/BTHET)*(((1./(ZIMP*XLNa(2))) - (1./(XLNa(1))))
